...
14. [Алгоритм рассадки](#14-алгоритм-рассадки)
15. [Модуль надзирателя](#15-модуль-надзирателя)
16. [Дополнительные бланки ответов](#16-дополнительные-бланки-ответов)
...
MinIO предоставляет S3-совместимое хранилище с тремя бакетами:

| Бакет | Содержимое | Формат имён объектов |
|---|---|---|
| `answer-sheets` | Сгенерированные PDF-бланки | `sheets/{competition_id}/{attempt_id}.pdf` |
| `scans` | Загруженные сканы | `scans/{scan_id}.{ext}` |
| `documents` | Документы участников (паспорт, военный билет) | `documents/{participant_id}/{document_id}.{ext}` |
...
Система содержит 14 таблиц: 8 основных + 6 новых (institutions, rooms, seat_assignments, documents, participant_events, answer_sheets).
...
│ created_at   │     │ institution_id  │──┐  │ variants_count │
│ updated_at   │     │   (FK, nullable)│  │  │ max_score      │
└──────────────┘     │ dob (Date)      │  │  │ status (enum)  │
       ...
       │      │                               │                     │
       │  ┌───▼───────────┐  ┌────────────────▼──────┐  ┌──────────▼────────┐
       │  │     scans     │  │   answer_sheets       │  │ participant_events│
       │  ├───────────────┤  ├───────────────────────┤  ├───────────────────┤
       │  │ id (UUID)     │  │ id (UUID)             │  │ id (UUID)         │
       │  │ attempt_id(FK)│  │ attempt_id (FK)       │  │ attempt_id (FK)   │
       │  │ answer_sheet_ │  │ sheet_token_hash      │  │ event_type (enum) │
       │  │   id (FK)     │  │   (unique)            │  │ timestamp         │
       │  │ file_path     │  │ kind (enum:           │  │ recorded_by (FK)  │
       │  │ ocr_score     │  │   primary/extra)      │  │ created_at        │
       │  │ ocr_confidence│  │ pdf_file_path         │  └───────────────────┘
       │  │ ocr_raw_text  │  │ created_at            │
       │  │ verified_by   │  └───────────────────────┘
       │  │ uploaded_by   │
       │  └───────────────┘
       │
       │  ┌─────────────────────┐     ┌─────────────────────┐
       │  │       rooms         │     │  seat_assignments   │
       │  ├─────────────────────┤     ├─────────────────────┤
       │  │ id (PK, UUID)       │◄────│ room_id (FK)        │
       │  │ competition_id (FK) │     │ registration_id     │
       │  │ name                │     │   (FK, unique)      │
       │  │ capacity            │     │ seat_number         │
       │  │ created_at          │     │ variant_number      │
       │  │ UQ(comp_id, name)   │     │ created_at          │
       │  └─────────────────────┘     │ UQ(room_id, seat_#) │
       │                              └─────────────────────┘
       │
       │  ┌───────────────┐     ┌───────────────┐
...
| `invigilator` | Надзиратель (контроль в аудитории) |
| `admin` | Полный доступ |
...
**EventType** - типы событий надзирателя:
| Значение | Описание |
|---|---|
| `start_work` | Начало работы участника |
| `submit` | Сдача работы |
| `exit_room` | Выход из аудитории |
| `enter_room` | Вход в аудиторию |

**SheetKind** - тип бланка ответа:
| Значение | Описание |
|---|---|
| `primary` | Основной бланк (итоговый балл считается с него) |
| `extra` | Дополнительный бланк (не влияет на балл) |
...
| Действие | Participant | Admitter | Scanner | Invigilator | Admin |
|---|:---:|:---:|:---:|:---:|:---:|
...
| Выдача бланков | - | + | - | - | + |
| Скачивание PDF бланка | - | + | - | + | + |
| Загрузка сканов | - | - | + | - | + |
| Верификация баллов | - | - | + | - | + |
| Фиксация событий участника | - | - | - | + | + |
| Выдача доп. бланков | - | - | - | + | + |
| Просмотр событий попытки | - | - | - | + | + |
...
1. Участник создаёт аккаунт (email, пароль, ФИО, учебное учреждение, класс, дата рождения)
2. Учебное учреждение выбирается из справочника с автодополнением
3. Участник может загрузить скан документа (паспорт / военный билет)
4. Входит в систему и получает JWT-токен
5. Регистрируется на олимпиаду
6. Система генерирует **Entry Token** (QR-код допуска):
...
Допуск поддерживает три режима сканирования QR: камера, загрузка изображения и **лазерный сканер** (USB HID устройство, вводит строку QR + Enter в текстовое поле).
...
5. На экране отображается: ФИО, учебное учреждение, дата рождения, наличие документов
6. При одобрении:
   - Entry token помечается как использованный
   - **Алгоритм рассадки** назначает аудиторию, место и вариант:
     - Участники одного учреждения распределяются по разным аудиториям
     - Вариант определяется по номеру места: `(seat_number % variants_count) + 1`
     - Результат сохраняется в `seat_assignments` (идемпотентно)
   - Генерируется **PDF-бланк ответов** с уникальным QR-кодом (sheet token)
   - Создаётся запись `AnswerSheet(kind=primary)` для бланка
   - PDF сохраняется в MinIO
   - Бланк выдаётся участнику (печать)
   - Создаётся запись Attempt в статусе `PRINTED`
   - На экране допускающего отображается: **аудитория**, **место**, **вариант**

#### Этап 6: Выполнение работы (Участник) + Контроль (Надзиратель)

Участник заполняет бланк ответов от руки, вписывает итоговый балл в специальное поле печатными цифрами.

**Надзиратель (Invigilator)** контролирует процесс в аудитории:
- Сканирует QR бланка участника (камера или лазерный сканер) для фиксации событий:
  - `start_work` — начало работы
  - `exit_room` — выход из аудитории (например, в туалет)
  - `enter_room` — возвращение в аудиторию
  - `submit` — сдача работы
- Все события сохраняются в таблице `participant_events` с временной меткой
- При необходимости надзиратель может выдать **дополнительный бланк ответов** (неограниченно):
  - Создаётся новая запись `AnswerSheet(kind=extra)` с уникальным QR-кодом
  - Итоговый балл считается ТОЛЬКО с основного бланка (`kind=primary`)
...
| Метод | Путь | Роль | Описание |
|---|---|---|---|
| POST | `/verify` | Admitter, Admin | Проверить QR допуска (+ учреждение, дата рождения, документы) |
| POST | `/{id}/approve` | Admitter, Admin | Одобрить, назначить аудиторию/место/вариант, выдать бланк |
| GET | `/sheets/{id}/download` | Admitter, Admin, Invigilator | Скачать PDF бланка |
...
### Учебные учреждения (`/api/v1/institutions`)

| Метод | Путь | Роль | Описание |
|---|---|---|---|
| GET | `/` | Публичный | Список учреждений (с пагинацией) |
| GET | `/search?q=` | Публичный | Поиск учреждений по названию |
| POST | `/` | Admin | Создать учреждение |
| DELETE | `/{id}` | Admin | Удалить учреждение |

### Аудитории (`/api/v1/rooms`)

| Метод | Путь | Роль | Описание |
|---|---|---|---|
| GET | `/{competition_id}` | Admin | Список аудиторий олимпиады |
| POST | `/{competition_id}` | Admin | Создать аудиторию |
| DELETE | `/room/{room_id}` | Admin | Удалить аудиторию |

### Надзиратель (`/api/v1/invigilator`)

| Метод | Путь | Роль | Описание |
|---|---|---|---|
| POST | `/events` | Invigilator, Admin | Зафиксировать событие (start_work, exit_room и др.) |
| GET | `/attempt/{id}/events` | Invigilator, Admin | Получить все события попытки |
| POST | `/extra-sheet` | Invigilator, Admin | Выдать дополнительный бланк ответов |

### Документы (`/api/v1/documents`)

| Метод | Путь | Роль | Описание |
|---|---|---|---|
| POST | `/` | Participant | Загрузить скан документа |
| GET | `/participant/{id}` | Admin, Admitter, Invigilator | Получить документы участника |
...
Допускающий (Admitter):
  /admission             Сканирование QR и допуск (камера / загрузка / лазер)

Надзиратель (Invigilator):
  /invigilator           Контроль участников, события, доп. бланки
...
Администратор (Admin):
  ...
  /admin/institutions    Справочник учебных учреждений
...
│   ├── admitter/         # Интерфейс допуска (камера/загрузка/лазер)
│   ├── invigilator/      # Модуль надзирателя (события, доп. бланки)
│   ├── scanner/          # Интерфейс сканирования
│   ├── admin/            # Панель администратора (+учреждения, аудитории)
│   └── public/           # Публичные результаты
├── components/           # Общие компоненты
│   ├── qr/               # QR-сканер, QR-отображение
│   ├── common/           # Button, Spinner, InstitutionAutocomplete и др.
...
|---|---|---|---|
| Unit | 79 | Нет | Доменная логика, сущности, value objects, алгоритм рассадки |
| Integration | 53 | SQLite in-memory | API-эндпоинты, use cases, репозитории |
...
- `admin_user` / `participant_user` / `invigilator_user` — предсозданные пользователи
- `institution` — предсозданное учебное учреждение
...
```bash
cd backend/
poetry run python -m pytest                         # Все 171 тест с покрытием
poetry run python -m pytest tests/unit -v            # Только unit (79)
poetry run python -m pytest tests/integration -v     # Только integration (53)
poetry run python -m pytest tests/e2e -v             # Только E2E (30)
poetry run python -m pytest --no-cov                # Без покрытия (быстрее)
```

---

## 14. Алгоритм рассадки

При допуске участника система автоматически назначает аудиторию, место и вариант. Алгоритм обеспечивает распределение участников одного учебного учреждения по разным аудиториям.

### Шаги алгоритма (`AssignSeatUseCase`)

```
1. Проверка идемпотентности
   └─ Если seat_assignment уже существует для registration_id → вернуть существующее

2. Получение данных
   ├─ institution_id участника (из Participant)
   └─ Список аудиторий олимпиады (из rooms)

3. Выбор аудитории
   ├─ Для каждой аудитории подсчитать:
   │   ├─ count_same_institution — кол-во участников из того же учреждения
   │   └─ free_seats = capacity - total_occupied
   ├─ Отфильтровать аудитории, где free_seats > 0
   ├─ Сортировка: min(count_same_institution), затем max(free_seats)
   └─ Выбрать первую аудиторию из отсортированного списка

4. Назначение места
   ├─ Получить занятые места в аудитории
   └─ Найти первый свободный номер (1, 2, 3, ...)

5. Назначение варианта
   └─ variant_number = (seat_number % variants_count) + 1

6. Сохранение
   └─ Создать SeatAssignment(registration_id, room_id, seat_number, variant_number)
       Уникальные ограничения: (room_id, seat_number) и (registration_id)
```

### Свойства алгоритма

- **Идемпотентность** — повторный вызов для той же регистрации возвращает тот же результат
- **Распределение по учреждениям** — минимизирует количество участников одного учреждения в одной аудитории
- **Детерминированные варианты** — вариант определяется номером места, обеспечивая равномерное распределение
- **Устойчивость к гонкам** — уникальные ограничения в БД предотвращают двойное назначение
- **Fallback** — если аудитории не настроены, вариант назначается случайно

---

## 15. Модуль надзирателя

### Роль `invigilator`

Надзиратель — сотрудник, контролирующий процесс в аудитории во время олимпиады. Имеет доступ к:
- Фиксации событий участников (выход/вход/начало/сдача)
- Просмотру истории событий для попытки
- Выдаче дополнительных бланков ответов
- Скачиванию PDF-бланков

### Поток работы надзирателя

```
┌─────────────────────────────────────────────────────────┐
│                 Модуль надзирателя                        │
│                                                          │
│  1. Сканирование QR бланка (камера / лазер)              │
│     └─ HMAC-SHA256(raw) → поиск attempt по хэшу          │
│                                                          │
│  2. Фиксация событий (кнопки):                           │
│     ├─ [Начало работы]  → event_type = start_work        │
│     ├─ [Выход]          → event_type = exit_room         │
│     ├─ [Вход]           → event_type = enter_room        │
│     └─ [Сдача работы]   → event_type = submit            │
│                                                          │
│  3. Просмотр истории событий участника                    │
│     └─ Таблица: событие, время, длительность вне ауд.    │
│                                                          │
│  4. Выдача доп. бланка (опционально):                    │
│     ├─ Генерация нового sheet_token (HMAC-SHA256)        │
│     ├─ Создание AnswerSheet(kind=extra, attempt_id)      │
│     └─ Отображение QR нового бланка на экране            │
└─────────────────────────────────────────────────────────┘
```

---

## 16. Дополнительные бланки ответов

### Модель данных

Для поддержки нескольких бланков на одного участника используется таблица `answer_sheets`:

```
answer_sheets
├─ id (PK, UUID)
├─ attempt_id (FK → attempts)     — связь с попыткой
├─ sheet_token_hash (unique)       — HMAC-хэш QR-кода бланка
├─ kind (enum: primary | extra)    — тип бланка
├─ pdf_file_path                   — путь к PDF в MinIO
└─ created_at
```

### Ключевые правила

1. При допуске (`approve_admission`) автоматически создаётся `AnswerSheet(kind=primary)`
2. Надзиратель может создать неограниченное количество `AnswerSheet(kind=extra)`
3. **Итоговый балл (`score_total`) считается ТОЛЬКО с основного бланка** (`kind=primary`)
4. Дополнительные бланки служат для ответов, но не влияют на результат
5. В модуле сканера бланки группируются по попытке: "Основной + N доп. бланков"
6. Привязка скана (`scans.answer_sheet_id`) позволяет точно определить, к какому бланку относится скан
